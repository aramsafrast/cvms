...
plot_confusion_matrix <- function(conf_matrix, target_col = "Target", prediction_col = "Prediction",
  ...
  checkmate::assert_data_frame(
    x = conf_matrix, any.missing = FALSE,
    min.rows = 4, min.cols = 3, col.names = "named", add = assert_collection
  )
  checkmate::assert_string(x = target_col, min.chars = 1, add = assert_collection)
  checkmate::assert_string(
    x = prediction_col, min.chars = 1,
    add = assert_collection
  )
  checkmate::assert_string(x = counts_col, min.chars = 1, add = assert_collection)
  checkmate::assert_string(
    x = tile_border_linetype, na.ok = TRUE,
    add = assert_collection
  )
  checkmate::assert_string(
    x = tile_border_color, na.ok = TRUE,
    add = assert_collection
  )
  checkmate::assert_string(
    x = palette, na.ok = TRUE, null.ok = TRUE,
    add = assert_collection
  )
  checkmate::assert_string(x = intensity_by, add = assert_collection)
  checkmate::assert_character(
    x = class_order, null.ok = TRUE,
  ...
  checkmate::assert_number(
    x = darkness, lower = 0, upper = 1,
    add = assert_collection
  )
  checkmate::assert_number(
    x = tile_border_size, lower = 0,
    add = assert_collection
  )
  checkmate::assert_number(x = arrow_size, lower = 0, add = assert_collection)
  checkmate::assert_number(
    x = arrow_nudge_from_text, lower = 0,
    add = assert_collection
  )
  checkmate::assert_count(x = digits, add = assert_collection)
  checkmate::assert_list(
    x = font_counts, names = "named",
    add = assert_collection
  )
  checkmate::assert_list(
    x = font_normalized, names = "named",
    add = assert_collection
  )
  checkmate::assert_list(
    x = font_row_percentages, names = "named",
    add = assert_collection
  )
  checkmate::assert_list(
    x = font_col_percentages, names = "named",
    add = assert_collection
  )
  checkmate::assert_list(
    x = sums_settings, names = "named",
    add = assert_collection
  )
  checkmate::reportAssertions(assert_collection)
  checkmate::assert_names(x = colnames(conf_matrix), must.include = c(
    target_col,
    prediction_col, counts_col
  ), add = assert_collection)
  available_font_settings <- names(font())
  checkmate::assert_names(
    x = names(font_counts), subset.of = available_font_settings,
    add = assert_collection
  )
  checkmate::assert_names(
    x = names(font_normalized), subset.of = available_font_settings,
    add = assert_collection
  )
  checkmate::assert_names(
    x = names(font_row_percentages),
    subset.of = available_font_settings, add = assert_collection
  )
  checkmate::assert_names(
    x = names(font_col_percentages),
    subset.of = available_font_settings, add = assert_collection
  )
  checkmate::assert_names(x = intensity_by, subset.of = c(
    "counts",
    "normalized"
  ), add = assert_collection)
  ...
  if (!is.null(sums_settings[["palette"]]) && palette == sums_settings[["palette"]]) {
  ...
  if (isTRUE(add_sums) && !isTRUE(user_has_ggnewscale) && is.null(sums_settings[["tile_fill"]])) {
  ...
  arrow_icons <- list(
    ...
    down = get_figure_path("caret_down_sharp.svg"), left = get_figure_path("caret_back_sharp.svg"),
  ...
  if (isTRUE(add_sums)) {
    ...
    sum_data[["N_text"]] <- preprocess_numeric(
      sum_data[["N"]],
      font_counts
    )
    sum_data[["Normalized_text"]] <- preprocess_numeric(
      sum_data[["Normalized"]],
      font_normalized
    )
    ...
    sums_color_limits <- get_color_limits(
      sums_intensity_measures,
      darkness
    )
  ...
  pl <- cm %>% ggplot2::ggplot(ggplot2::aes(
    ...
    ggplot2::geom_tile(
      ...
      linewidth = tile_border_size, linetype = tile_border_linetype,
      show.legend = FALSE
    ) +
    theme_fn() +
    ggplot2::coord_equal() +
    ...
    ggplot2::theme(
      ...
      axis.title.y = ggplot2::element_text(margin = ggplot2::margin(
        0,
        6, 0, 0
      )), axis.title.x.top = ggplot2::element_text(margin = ggplot2::margin(
        0,
        0, 6, 0
      )), axis.title.x.bottom = ggplot2::element_text(margin = ggplot2::margin(
        6,
        0, 0, 0
      ))
    )
  if (isTRUE(add_sums)) {
    if (is.null(sums_settings[["tile_fill"]])) {
      pl <- pl + ggnewscale::new_scale_fill() + ggplot2::geom_tile(
        ...
        direction = 1, limits = sums_color_limits
      )
    } else {
  ...
  if (isTRUE(place_x_axis_above)) {
    pl <- pl + ggplot2::scale_x_discrete(
      position = "top",
